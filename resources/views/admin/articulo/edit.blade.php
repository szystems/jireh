                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ url('update-articulo/'.$articulo->id) }}" method="POST" id="form-articulo">
                                ...
                                <!-- Sección colapsable para componentes si es un servicio -->
                                @if($articulo->tipo == 'servicio')
                                <div class="accordion mb-4" id="accordionComponentes">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const seccionMecanico = document.querySelector('#seccion-mecanico'); // Añadir esta referencia
    ...
    tipoSelect.addEventListener('change', function() {
        ...
        // Buscar todos los elementos posibles relacionados con componentes
        const seccionComponentes = document.querySelector('#seccion-componentes');
        const accordionComponentes = document.querySelector('#accordionComponentes');
        const componentesTabPane = document.querySelector('#componentes-tab-pane');

        console.log("Elementos encontrados:", {
            seccionMecanico: seccionMecanico !== null,
            seccionComponentes: seccionComponentes !== null,
            accordionComponentes: accordionComponentes !== null,
            componentesTab: componentesTab !== null,
            componentesTabPane: componentesTabPane !== null
        });
        ...
        if (this.value === 'servicio') {
            ...
            if (accordionComponentes) accordionComponentes.style.display = 'block';
            if (componentesTab) componentesTab.classList.remove('d-none');
            if (componentesTabPane) componentesTabPane.classList.remove('d-none');

            console.log("Mostrando secciones para tipo SERVICIO");
            ...
            if (componentesTab && typeof bootstrap !== 'undefined') {
                try {
                    ...
                    console.log("Pestaña de componentes activada");
                } catch(e) {
            ...
            if (accordionComponentes) accordionComponentes.style.display = 'none';
            if (componentesTab) componentesTab.classList.add('d-none');
            if (componentesTabPane) componentesTabPane.classList.add('d-none');

            console.log("Ocultando secciones para tipo ARTICULO");
        }
    ...
    // Ejecutar el evento change al cargar la página para establecer la visibilidad inicial
    setTimeout(function() {
...
document.addEventListener('DOMContentLoaded', function() {
    // Escuchar cambios en los campos de comisión
    ...
    // Inicializar tabla de margen si no existe
    const margenDetalle = document.getElementById('margen-detalle');
    if (margenDetalle && !margenDetalle.querySelector('table')) {
        margenDetalle.innerHTML = `
            <h6 class="mb-3">Margen de Ganancia</h6>
            <table class="table table-sm">
                <tbody>
                    <tr>
                        <td class="text-start">Precio de venta</td>
                        <td class="text-end" id="td-precio-venta">$0.00</td>
                    </tr>
                    <tr>
                        <td class="text-start">Precio de compra</td>
                        <td class="text-end" id="td-precio-compra">- $0.00</td>
                    </tr>
                    <tr>
                        <td class="text-start" id="td-label-impuesto">Impuesto (0%)</td>
                        <td class="text-end" id="td-impuesto">- $0.00</td>
                    </tr>
                    <tr>
                        <td class="text-start fw-bold">Ganancia real</td>
                        <td class="text-end text-success" id="td-ganancia-real">$0.00</td>
                    </tr>
                </tbody>
            </table>
    ...
    // Inicialización para mostrar/ocultar sección de mecánico según tipo inicial
    const tipoSelect = document.querySelector('#tipo');
    const componentesTab = document.querySelector('#componentes-tab');
    const seccionMecanico = document.querySelector('#seccion-mecanico');
    const accordionComponentes = document.querySelector('#accordionComponentes'); // Referencia al acordeón

    if (tipoSelect && componentesTab && seccionMecanico) {
        if (tipoSelect.value === 'servicio') {
            componentesTab.classList.remove('d-none');
            seccionMecanico.style.display = 'block';
            if (accordionComponentes) accordionComponentes.style.display = 'block'; // Mostrar acordeón
        } else {
            componentesTab.classList.add('d-none');
            seccionMecanico.style.display = 'none';
            if (accordionComponentes) accordionComponentes.style.display = 'none'; // Ocultar acordeón
        }
    } else {
        console.error('No se encontraron todos los elementos necesarios:',
            {tipoSelect: !!tipoSelect, componentesTab: !!componentesTab, seccionMecanico: !!seccionMecanico});
    }
});
</script>
@endsection
